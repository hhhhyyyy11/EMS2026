...
#0710 無駄電力を復活．入力データを曇りの日も含むものに変更(それにともない買電価格のデータも変更)
...
excel_path = '20250417_20250506テクシード石井工場トレンドデータ.xlsx'
...
pBY = df_trend['売買電価格'].iloc[:K].ffill() #0702変更 売買に変更
pBY = pBY.tolist()

dD = df_trend['本社工場デマンド'].iloc[:K].ffill() #0702変更
dD = dD.tolist()

pD = 0.001 #本社工場へ託送することのインセンティブ係数
...
v = {k: model.addVar(vtype='C', name=f'v_{k}', lb=0) for k in range(K)} #0710修正 無駄電力を復活
...
model.setObjective(sum( pBY[k] * sBY[k] - pD*dD2[k] for k in range(K)), 'minimize') #0702修正

# 制約の追加（17本すべてをPDFの順序通りに）
for k in range(K):
    # 電力バランス制約
    model.addCons(gP2[k] + sBY[k] - xFC1[k] + xFD2[k] - dA1[k] - dB1[k] - dC1[k] - dD1[k] - v[k] == 0)#0710 vを復活 
    if J_P > 1:
        for j in range(J_P):
...
for k in range(K):
    results.append([
        ...
        dB2[k] * 60,model.getVal(dC1[k]) * 60,dC2[k] * 60,model.getVal(sBY[k]) * 60,model.getVal(xFC1[k]) * 60,
...
for i, var in enumerate(variables):
    ...
    ax.plot(df_results.index, df_results[var],label = var)  # , marker='o', linestyle='-')
...
# PDFを保存
pdf_output_path = 'Optimization_Results_買電売電を考慮.pdf'
