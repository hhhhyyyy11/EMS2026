...
状態 $s_{i}(\in S_{k-1})$から$ s_{j} (\in S_{k})$への遷移に伴う評価関数を$w_{k-1}(s_{i},s_{j})$とする．系統から買う電力量が$a^{\rm BY}$であるような
状態遷移のみを許容する場合，問題は以下のように定式化される．
\begin{equation}
    \text{Minimize} \quad \{ \sum_{k=0}^{K-1} w_k(s_i,s_j) | s_i \in S_k, s_j \in S_{k+1},\frac{j - i}{B-1}\overline{b} \leq a^{\rm BY} \}
 \label{eq:minimize}
\end{equation}
...
\begin{equation}
 s_k^{\rm BY} = 
 d_k^{\rm A1}+ d_k^{\rm B1} + d_k^{\rm C1}+  x_k^{\rm FC1}- g_k^{\rm P2} \\
  \label{eq:pass_abst}
\end{equation}
...
\begin{figure}[h]
 \centering
 \includegraphics[width=140mm]{../figure/tecseed20251002.png}
...
\]
\[
F_{k}(s_{i}) = \min_{s_{j} \in S_{k-1}, \frac{i-j}{B-1}\overline{b} \leq a^{\rm BY} } ( F_{k-1}(s_{j}) + w_{k-1}(s_{j},s_{i}) )  ,\quad (1 \leq k \leq K) \\
 ．
 \label{eq:minimize}
\]
この更新式に則って $F_{0}$ から $F_{K}$ まで順次に値を求めることで，最終的な状態における最小の評価値と，その最小の評価値を得るためには，各々の期でどのように状態を遷移すればよいのかを求めることができる．


\subsection{バッテリーの刻み幅} 
 本アプローチにおける計算量は，$O(B^2 K)$となる．
$K$については2週間程度の電力運用計画をつくりたい」という要望があり調節をすることができないため，
現実的な計算時間内で最適な解を得るためには，バッテリーの段階の個数である$B$をどの程度の大きさに設定するかが肝要となる． 
 買電については「30分間の買う電力が50kW以下という制約条件を加えたい」という要求があるため，$B$の値が小さすぎる場合，
...
\section{実験}
2025年4月17日木曜日から4月26日金曜日までの9日間について実験を行った．4月18日金曜日の午後5時から4月21日月曜日の午前8時までと，4月21日月曜日の午後5時から
4月23日の午前8時までは，テクシード石井工場側が太陽光発電を意図的に停止させていることに留意する．
なお$p^{\rm BY}_k$の値については，日本卸電力取引所の30分ごとの約定価格のデータ（\url{https://www.jepx.jp/electricpower/market-data/spot/}，2025年6月17日アクセス）を参照して決定した．
...
\section{参考資料}
同様の期間について，数理計画を用いて得られた結果を図\ref{fig:optimization_results_lp_p3}に示す．
\begin{figure}[h]
 \centering
 \includegraphics[width=1.1\textwidth, page = 1]{Optimization_Results_LP.pdf}
 \label{fig:optimization_results_lp_p1}
\end{figure}

\begin{figure}[h]
 \centering
 \includegraphics[width=1.1\textwidth, page = 2]{Optimization_Results_LP.pdf}
 \label{fig:optimization_results_lp_p2}
\end{figure}

\begin{figure}[h]
 \centering
 \includegraphics[width=1.1\textwidth, page = 3]{Optimization_Results_LP.pdf}
   \caption{最適化結果(数理計画)}
 \label{fig:optimization_results_lp_p3}
\end{figure}
