...
~ (定数を「\IB」，変数を「\IC」を付して記すものとする)}}
 ...
 \item[\IB] 消費電力C (変換前) $d^{\rm C1}_{k}$
  ...
  \item[\IC] 状態 $i\in S_{k-1}$から状態$j\in S_k$へ遷移するときの固定バッテリーの充電量(変換前) $x^{\rm FC1}_{k}(i,j)$
  \\ただし，$x^{\rm FC1}_{k}(i,j) < 0$を満たしているときは$|x^{\rm FC1}_{k}(i,j)|$だけ放電を行っていると解釈する．
  \item[\IC] 状態 $i \in S_{k-1}$から状態$j \in S_{k}$へ遷移するときの固定バッテリーの充電量(変換後) $x^{\rm FC2}_{k}(i,j)$
  ...
  %
  \item[\IC]  状態 $i \in S_{k-1}$から状態$j \in S_{k}$へ遷移するときの系統買電力量$s^{\rm BY}_{k}(i,j)$  \\ただし，$s^{\rm BY}_{k}(i,j)< 0$を満たしているときは$|s^{\rm BY}_{k}(i,j) |$だけ系統に売電を行っていると解釈する．
...
\begin{equation}
 x^{\rm FC2}_{k}(i,j) =\begin{cases}
  ...
  \frac{1}{\alpha^{\rm FD}}x^{\rm FC1}_{k}(i,j) \quad \text{otherwise }
 \end{cases}
  \hfill
 ,k=1,\ldots,K
 \label{eq:xFC1FC2}
...
\subsection{定式化}
バッテリーのSOCとしてとりうる値を，0が最小値， $\overline{b}$が最大値となるように，等間隔に $B( \geq 2)$段階で離散化する．これにより，期 \(k\) の末尾におけるSOCは
$0,\ \frac{1}{B-1}\overline{b},\ \frac{2}{B-1}\overline{b},\ \dots,\ \overline{b}$のいずれかをとる．
便宜上バッテリーの刻み幅$\Delta b$を$\Delta b = \frac{\overline{b}}{B-1}$と定義する．この定義によりバッテリーのSOCとして取りうる値は
$0,\Delta b,2\Delta b,\ \dots,(B-1)\Delta b$
と表される．
また，各々の期について，バッテリーのSOCが上記の値になっている状態それぞれを名前をつけて区別すべく，
状態を要素に持つ集合$S_{0},S_{1},\dots,S_{K-1}$を以下のように定義する：
\begin{gather}
S_k =  \{  i_0 \} ,\quad  k = 0 \\
S_k = \{ 0, 1, \dots, B - 1 \} ,\quad  k  = 1, \dots , K
 \label{eq:state}
\end{gather}

状態$i\in S_{k}$は以下の式を満たしている状態を表すことにする：
\begin{equation}
b_k = i \Delta b .
\label{eq:B_K}
\end{equation}

次に，状態$i\in S_{k-1}$から状態$j \in S_{k}$へ遷移可能であることを満たす添字$i,j$を要素としてもつ集合
$T^{\rm BY}_{0},T^{\rm BY}_{1},\dots,T^{\rm BY}_{K-1}$を定義することを考える．
そのために，集合$T^{\rm BY}_{k}$を$-a^{\rm SL} \le s^{\rm{BY}}_{k}(i,j) \le a^{\rm BY}$である状態遷移の集合を表すよう，次のように定義する：
\begin{equation}
 T^{\rm BY}_{k} =\{(i,j) \in \mathbb{Z}^2 | 1 \le k \le K, i\in S_{k-1},j \in S_{k}, s_k^{\rm{MinIDX}} \le j-i \le s_k^{\rm{MaxIDX}} \} ．
\label{eq:T_BY}
\end{equation}
$ s_k^{\rm{MinIDX}}$と$s_k^{\rm{MaxIDX}}$の設定方法については後述する．

状態 $i\in S_{k-1}$から状態$j\in S_{k}$への遷移に伴う評価関数を$w_k(i,j)$とする．この評価関数は$(i,j) \in  T^{\rm BY}_{k}$を満たすときに
...
\begin{equation}
    \text{Minimize} \quad  \sum_{k=1}^{K-1} w_k(i,j) \quad  (i,j)\in T^{\rm BY}_{k}
 \label{eq:minimize}
\end{equation}


以下，評価関数の計算方法について述べる．工場を管理する人目線で考えると，お金の減少量をなるべく抑えることが望ましいと考えられる．そのことに留意すると，評価関数 $w_{k}(i,j)$の表し方のひとつとして以下のようなものが考えられる．ここで，集合$T^{\rm PBY}_{k}$は，$s^{\rm{BY}}_{k}(i,j) \ge 0$である状態遷移の集合を表す．$ s_k^{\rm{PBYMinIDX}}$と$s_k^{\rm{PBYMaxIDX}}$の設定方法については後述する．
\begin{equation}
  ...
  w_{k}(i,j) = \begin{cases}
   p_k^{\rm BY}s_k^{\rm BY}(i,j),\quad   \text{if }(i,j) \in T^{\rm PBY}_{k}\\
...
\begin{equation}
	T^{\rm PBY}_{k} =\{(i,j) \in \mathbb{Z}^2 | s_k^{\rm{PBYMinIDX}} \le j-i \le s_k^{\rm{PBYMaxIDX}} \}
 \label{eq:PBY}
\end{equation}
...
\begin{equation}
 s_k^{\rm BY}(i,j) = \begin{cases}
	\frac{1}{\alpha^{\rm FC}}x_k^{\rm FC2}(i,j) - r_k ，\text{if } j-i \geq 0\\
...
\begin{equation}
 ...
 \label{eq:x_kFC2}
\end{equation}
と表される．以上より，(\ref{eq:weight})の右辺の項は全て定数となり，$(i,j)\in T^{\rm BY}_{k}$を満たすすべての状態遷移について評価関数を設定することができる．
...
期1から期$k$までの全体の評価関数を$f_{k}(w_1,w_2,\dots,w_{k})$とする．
このとき，$f_{k}$は
\[
f_{k} = w_1+w_2+\dots+w_{k-1}+w_{k}=f_{k-1} + w_{k}
\]
と表され，分離可能性を満たす．また，$w_{k}$は単調増加関数であるため，$f_{k}$は$f_{k-1}$について単調であり単調性も満たす．
よって，本問題は，”最適経路の部分経路もまた最適でなければならない”という最適性の原理を満たしており，動的計画法により解くことができる．
ここで$F_{k}^{*}(i)$は状態$0\in S_{0}$から状態$i \in S_{k}$までを最も評価値の和が低くなるように遷移したときの評価値を表す．つまり，
\begin{equation}
	F_{k}^{*}(i_{k}) =  \min \sum_{l=1}^{k} w_l(i,j) \quad  (i,j)\in T^{\rm BY}_{l}
	\label{eq:F_k*}
\end{equation}
と表される．このときBellman方程式は，
\begin{equation}
	F_{k}^{*}(i_{k}) = \min_{(j,i) \in T_{k}^{\rm{BY}} } \{ w_{k}(j,i) + F_{k-1}^{*}(j)\}
 	\label{eq:minimize}
\end{equation}
これに則って順番に$F_{k}^{*}(i)$の値を求めることで，期$K$の状態における評価値のうち最小のものと，その最小の評価値を得るためには，各々の期でどのように状態を遷移すればよいのかを求めることができる．
