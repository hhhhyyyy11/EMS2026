...
~ (定数を「\IB」，変数を「\IC」を付して記すものとする)}}
 ...
 \begin{itemize}
  ...
  \item[\IC] 買電と売電を排他的にするためのスイッチ変数 $y_k \in \{1,0\}$
...
前項で示した定数・変数を用いて，電力運用最適化問題の線形計画モデルを示す．なお，式(\ref{eq:sBYexclusive})，(\ref{eq:sSLexclusive})
における$M$は十分大きな正の定数を表す．
%
...
%
\begin{equation}
s^{\rm BY}_{k} \le My_{k}
 \hfill
 k=1,\ldots,K
~~
 \label{eq:sBYexclusive}
\end{equation}
\begin{equation}
s^{\rm SL}_{k} \le M(1-y_{k})
 \hfill
 k=1,\ldots,K
~~
 \label{eq:sSLexclusive}
\end{equation}
...
なお，式(\ref{eq:minimize})における，$p^{\rm BY}_k$の値については，任意の$k$について26.11とした．この値は四国電力株式会社の公式ウェブサイト\footnote{四国電力「ご契約番号(10桁)をご案内しているお客様および新規にご契約いただくお客様」，\\ \url{https://www.yonden.co.jp/business/price/plan/10.html}，2025年6月17日アクセス）}を参照して設定したものである．具体的には，契約電力が500kW未満かつその他期(7〜9月以外)の場合の電力量料金単価28.47円/kWhから，燃料費調整単価の-6.34円/kWhと，再エネ賦課金の3.98円/kWh\footnote{四国電力「再生可能エネルギー発電促進賦課金・単価表」，\url{https://www.yonden.co.jp/extra/renewable.html}（2025年5月14日アクセス）}を加えた，28.47-7.52+3.49=24.44円/kWhという値を参照している．
次に，$p^{\rm SL}_k$については，日本卸電力取引所の30分ごとの約定価格のデータ（\url{https://www.jepx.jp/electricpower/market-data/spot/}，2025年6月17日アクセス）
を参照して決定した．また，式(\ref{eq:sMax})における$a^{\rm BY}$の値については，神戸大学 共同研究検討依頼資料2025.0516.xlsxを参考に「契約電力が50kW以下にな
らなければならない」という制約になるように設定した．
%28.47-7.52+3.49=24.44
...
\section{結果と考察}
本最適化モデルでは求解するのに要した時間は156.50秒であった．これに対し，本最適化モデルから式(\ref{eq:sBYexclusive})，(\ref{eq:sSLexclusive})
をなくしたモデルでは求解するのに要した時間は19.93秒であり，買電と売電を排他的にする制約を加えることにより計算時間が7.8倍程度増加した．

本最適化モデルによって得られた解でのバッテリーSOCを，実際のバッテリーSOCと比較すると，
実際のバッテリーSOCではSOCが最終的に100\%になっているのに対し，本最適化モデルではSOCが最終的に0\%になっていることがわかる．
これは，系統から電力を購入する代わりにすでに溜まっている蓄電池の電力を使い切ってしまおうとしていることが原因であると考えられる．
