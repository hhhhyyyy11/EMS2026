...
excel_path = '20250703_20250721テクシード石井工場トレンドデータ.xlsx'
...
# 定数の設定
gP2 = df_trend['太陽光発電電力'].iloc[:K].ffill()
gP2 = (gP2.values / 60) / 1.2
gP2[gP2 < 0] = 0  # 負の値を0に変換
...
# 下限をなくして、マイナスも許容する
v = {k: model.addVar(vtype='C', name=f'v_{k}', lb=-model.infinity()) for k in range(K)}
...
model.setObjective(sum( 1 for k in range(K)), 'minimize') #0702修正
...
for k in range(K):
    ...
    #実際どおりに充放電や買電をする
    if xFD1_xFC2_actual[k] >= 0:
        model.addCons(xFD1[k] == xFD1_xFC2_actual[k])
        model.addCons(xFC2[k] == 0)
    else :
        model.addCons(xFC2[k] == -xFD1_xFC2_actual[k])
        model.addCons(xFD1[k] == 0)
    if s_actual[k] >= 0:
        model.addCons(sBY[k] == s_actual[k])
        model.addCons(sSL[k] == 0)
    else:
        model.addCons(sBY[k] == 0)
        model.addCons(sSL[k] == -s_actual[k])

    #買電制約
    #if k <= K - 30 + 1:
    #    model.addCons(sum(sBY[k2] for k2 in range(k,k+30-1) ) <= sBYMAX)
...
for k in range(K):
    results.append([
        k,
        gP2[k] * 60,dA2[k] * 60,
        dB2[k] * 60,dC2[k] * 60,model.getVal(sBY[k]) * 60,model.getVal(xFC1[k]) * 60,
        model.getVal(xFC2[k]) * 60,model.getVal(xFD1[k]) * 60,model.getVal(xFD2[k]) * 60,model.getVal(bF[k]),bF_actual[k],
        s_actual[k] * 60,model.getVal(xFD1[k]) * 60 - model.getVal(xFC2[k]) * 60,xFD1_xFC2_actual[k] * 60,solar_radiation[k],
        model.getVal(v[k])*60
    ])
...
columns = ['k', 
           'gP2','dA2',
           'dB2', 'dC2', 'sBY,sSL','xFC1', 
           'xFC2', 'xFD1', 'xFD2', 'bF','bF_actual',
            's_actual', 'xFD1_xFC2', 'xFD1_xFC2_actual', 'solar_radiation',
            'v']
unit = ['[kW]','[kW]', 
        '[kW]', '[kW]', '[kW]', '[kW]', 
        '[kW]', '[kW]', '[kW]','[kWh]', '[kWh]', 
        '[kW]', '[kW]', '[kW]', '[W/m2]',
        '[kW]']
name = ['太陽光発電電力量(変換後)','消費電力A(変換後)',
         '消費電力B(変換後)','消費電力C(変換後)', '系統買電','バッテリー充電量(変換前)', 
         'バッテリー充電量(変換後)', 'バッテリー放電量(変換前)', 'バッテリー放電量(変換後)','バッテリーSOC', '実際のバッテリーSOC', 
         '実際の系統買電', 'バッテリー放電量/充電量','実際のバッテリー放電量/充電量', '日射量',
         '無駄電力量'
        ]
ymin = [0, 0, 
        0, 0, -250, 0, 
