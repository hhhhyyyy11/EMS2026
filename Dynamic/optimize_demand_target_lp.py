# -*- coding: utf-8 -*-
"""Demand-target LP model with monthly rolling demand-charge variables.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="LP optimization with monthly demand-target variables."
    )
    parser.add_argument("--config", default="config.json", help="Path to config.json")
    parser.add_argument("--sheet", default="30分値", help="Excel sheet name")
    parser.add_argument(
        "--output-dir",
        default="Dynamic/results/demand_target_lp",
...
def load_lp_result_grid_purchase_kw(csv_path: str, expected_length: int) -> List[float]:
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            ...
            "Run Dynamic/optimize_LP.py first, or use --past-grid-source net_demand."
        )
...
def solve_demand_target_lp(
    ...
    s_bar = {
        month: model.addVar(vtype="C", name=f"S_bar_{month_label(month)}", lb=0.0)
    ...
    for k in range(k_future):
        ...
        if battery.capacity_kwh <= 0.0:
            ...
            model.addCons(charge_input[k] <= big_m * charge_mode[k])
            model.addCons(discharge_from_battery[k] <= big_m * (1 - charge_mode[k]))

    for month, refs in reference_sets.items():
        for past_k in refs["past"]:
            model.addCons(past_grid_purchase_kw[past_k] <= s_bar[month])
        for future_k in refs["future"]:
            model.addCons(s_by[future_k] <= s_bar[month])

    basic_cost = quicksum(
        tariff.basic_rate_yen_per_kw_month * s_bar[month] for month in future_months
    )
    ...
    if model.getNSols() == 0:
        raise RuntimeError(f"No feasible solution found. SCIP status: {status}")
    ...
    s_bar_values = {month: model.getVal(s_bar[month]) for month in future_months}
...
def summarize_monthly(future_df, result: dict, tariff: TariffParams):
    ...
    for month in result["future_months"]:
        ...
        demand_target = float(result["s_bar"][month])
        basic_cost = tariff.basic_rate_yen_per_kw_month * demand_target
        rows.append(
            {
...
def main() -> None:
    ...
    print(f"Max monthly demand target: {summary['max_monthly_demand_target_kW']:.2f} kW")
