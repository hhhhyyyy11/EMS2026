...
alpha_FD = {j: 0.94 for j in range(J_FD)}
...
for k in range(K):
    ...
    # バッテリーSOC更新式
    model.addCons(bF[k] == (0.99999999999*bF[k - 1] + xFC2[k] - xFD1[k]) if k > 0 else bF[k] == (bF0 + xFC2[k] - xFD1[k]))
...
# ==========================================
print("最初の12時間の詳細グラフを作成しています...")

# 1. 開始時刻と終了時刻（開始から12時間後）を定義
start_dt = df_results.index[0]
end_dt = start_dt + pd.Timedelta(hours=12)
...
# 4. PNGとして保存
output_png_12h = 'png/grid_power_first_12h.png'
...
# ==========================================
# 追加機能: 最初の12時間だけの売電単価グラフを出力
# ==========================================
print("最初の12時間の売電単価グラフを作成しています...")

# 1. データの切り出し（既に定義済みの start_dt, end_dt を使用）
...
# 'pSL' (売買電単価) 列をプロット
ax2.plot(df_subset_price.index.to_pydatetime(), df_subset_price['pSL'].values, color='green', linewidth=2.0)
...
plt.xlabel('Time')
plt.title(f'Selling Price (First 12h from {start_dt.strftime("%Y-%m-%d")})')
plt.grid(True)

# 3. PNGとして保存
output_png_price_12h = 'png/selling_price_first_12h.png'
plt.tight_layout()
plt.savefig(output_png_price_12h, dpi=300)
plt.close(fig2)
